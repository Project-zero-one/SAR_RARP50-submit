...
class ResNet3dSlowFast(nn.Module):
    ...
    Example:
        >>> from mmseg.models import ResNet3dSlowFast
        >>> import torch
        >>> self = ResNet3dSlowFast(pretrained=None)
        >>> self.eval()
        >>> inputs = torch.rand(1, 3, 16, 512, 512)  # (N,C,T,H,W)
        >>> level_outputs = self.forward(inputs)
        >>> for level_out in level_outputs:
        ...     print(tuple(level_out[0].shape), tuple(level_out[1].shape))
        (1, 320, 2, 128, 128) (1, 32, 16, 128, 128)  # slow pathway, fast pathway
        (1, 640, 2, 64, 64) (1, 64, 16, 64, 64)
        (1, 1280, 2, 32, 32) (1, 128, 16, 32, 32)
        (1, 2048, 2, 16, 16) (1, 256, 16, 16, 16)
    """
    ...
    def forward(self, x):
        ...
        x_slow = nn.functional.interpolate(
            x.flip(2),  # for sampling from end
            mode='nearest',
            scale_factor=(1.0 / self.resample_rate, 1.0, 1.0)).flip(2)
        ...
        x_fast = nn.functional.interpolate(
            x.flip(2),  # for sampling from end
            mode='nearest',
            scale_factor=(1.0 / (self.resample_rate // self.speed_ratio), 1.0, 1.0)).flip(2)
