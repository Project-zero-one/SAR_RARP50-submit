...
def train_segmentor(model,
    ...
    # register eval hooks
    if validate:
        if isinstance(cfg.data.val, list):
            # overwirte train pipeline at index 0
            pipeline = cfg.data.val[0].pipeline
            for i in range(len(cfg.data.val)):
                cfg.data.val[i].pipeline = pipeline
        elif cfg.data.val.type == 'ConcatDataset':
            # add key `pipeline` each dataset
            pipeline = cfg.data.val.pipeline
            for i in range(len(cfg.data.val.datasets)):
                cfg.data.val.datasets[i].update(dict(pipeline=pipeline))
