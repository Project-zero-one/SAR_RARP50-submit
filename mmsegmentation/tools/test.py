...
def change_dump_path(cfg_data, parent_dir):
    if isinstance(cfg_data, list):
        ...
        if cfg_data.get('dump_path') and not osp.isabs(cfg_data['dump_path']):
...
def main():
    ...
    cfg.data.test = change_dump_path(cfg.data.test, args.work_dir)
    if isinstance(cfg.data.test, list):
        ...
        for i in range(len(cfg.data.test)):
            cfg.data.test[i].pipeline = pipeline
    elif cfg.data.test.type == 'ConcatDataset':
        # add key `pipeline` each dataset
        pipeline = cfg.data.test.pipeline
        for i in range(len(cfg.data.test.datasets)):
            cfg.data.test.datasets[i].update(dict(pipeline=pipeline))
