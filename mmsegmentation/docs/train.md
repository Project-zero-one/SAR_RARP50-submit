...
### Tensorboard
`logdir` needs `/` at end position.

```shell
docker exec -it mmseg-yamada-gpu1 tensorboard --port 58829 --host=0.0.0.0 --logdir /path/to/tf_logs/
```
http://localhost:58829


if you only use tensorboard, you can use tensorflow image like bellow:
```shell
docker run \
    -td \
    --rm \
    ...
    tensorboard --port 5665 --host=0.0.0.0 --logdir /mnt/cloudy_z/src/atsushi/mmsegmentation/work_dirs/
```
http://localhost:5665