...
```shell
cd docker/
docker build -t mmsegmentation:pytorch1.6.0-cuda10.1-cudnn7-mmcv1.3.13 .
```
*note: Recommend checking this docker image existing or not. If you have already built the image, you can skip this part.

2. Run container

`PORT` is own port number for tensorboard. You should choose `PORT` number not to double others.

`--name` option is your container name. You should give a understanding name so that you don't double the name.

*Naming Rule: `mmseg-{yourname}-gpu{gpu-id}`
...
docker run \
   ...
   -itd \
   -p PORT:PORT \
   --name mmseg-yamada-gpu1 \
   -v /mnt:/mnt \
   mmsegmentation:pytorch1.6.0-cuda10.1-cudnn7-mmcv1.3.13
```
...
docker run \
   ...
   mmsegmentation:pytorch1.6.0-cuda10.1-cudnn7-mmcv1.3.13
```
*note: If you use multi GPU container, mmseg may use cuda:0 on validation absolutely.

3. Install your mmsegmentation repository
```shell
docker exec -it mmseg-yamada-gpu1 bash
...
*If you can't import mmcv(or cv2), run below
```shell
docker exec -it mmseg-yamada-gpu1 'apt update && apt install -y libgl1-mesa-dev'
```
