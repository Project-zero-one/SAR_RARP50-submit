...
from mmaction.core.evaluation import mean_class_accuracy, confusion_matrix
...
def main():
    ...
    video_infos = dataset.load_annotations()
    y_real = [video_info['label'] for video_info in video_infos]
    video_names = [osp.basename(osp.dirname(video_info['data_path'])) for video_info in video_infos]
    pred_frame_inds = [video_info['frame_ind'] for video_info in video_infos]
    ...
    y_pred = [np.argmax(prob) for prob in outputs]

    # confusion matrix
    mat = confusion_matrix(y_pred, y_real)
    ...
    if rank == 0:
        if output_config.get('out', None):
            ...
            plot_confusion_matrix(mat, np.unique(y_real), save_dir=osp.dirname(out), color_theme='GnBu')
            dump_prob_results(outputs, video_names, pred_frame_inds, osp.dirname(out))
            dump_class_results(y_pred, y_real, video_names, pred_frame_inds, osp.dirname(out))

        if eval_config:
            eval_res = dataset.evaluate(outputs, **eval_config)
            eval_res.update({'mean class accuracy': mean_class_accuracy(outputs, y_real)})
            for name, val in eval_res.items():
                print(f'{name}: {val:.04f}')

            with open(osp.join(osp.dirname(out), 'eval.json'), 'w') as f:
                json.dump(eval_res, f, indent=2)


def dump_prob_results(outputs, video_names, pred_frame_inds, save_dir_path):
    results = defaultdict(list)
    unique_video_names = list(set(video_names))
    _video_names = np.array(video_names)
    _pred_frame_inds = np.array(pred_frame_inds)
    _outputs = np.array(outputs)

    for video_name in unique_video_names:
        video_outputs = _outputs[video_names == video_name]
        video_pred_frame_inds = _pred_frame_inds[video_names == video_name]

        for frame_idx, prob in zip(video_pred_frame_inds, video_outputs):
            results[video_name].append({f'{str(frame_idx):09d}': prob})

    with open(osp.join(save_dir_path, 'video_prob.json'), 'w') as f:
        json.dump(results, f)


def dump_class_results(y_pred, y_real, video_names, pred_frame_inds, save_dir_path):
    results = defaultdict(list)
    unique_video_names = list(set(video_name))
    _video_names = np.array(video_names)
    _pred_frame_inds = np.array(pred_frame_inds)
    _y_pred = np.array(y_pred)
    _y_real = np.array(y_real)

    for video_name in unique_video_names:
        video_y_pred = _y_pred[video_names == video_name]
        video_y_real = _y_real[video_names == video_name]
        video_pred_frame_inds = _pred_frame_inds[video_names == video_name]
        video_pred_frame_inds = [f'{str(frame_idx):09d}' for frame_idx in video_pred_frame_inds]

        results[video_name].append({f'frame_index': video_pred_frame_inds})
        results[video_name].append({f'pred': video_y_pred})
        results[video_name].append({f'real': video_y_real})

    with open(osp.join(save_dir_path, 'video_pred_real_class.json'), 'w') as f:
        json.dump(results, f)
